function [ testFeat, trainFeat ] = crossValidation(FEAT, disorderTruth, K )
%UNTITLED2 separates features and evaluates via cross validation
%   Detailed explanation goes here
FEAT = FEAT(1:250,:);
disorderTruth = disorderTruth(1:250);
% n = number of folds
n = 10;
% s = size of fold
...
for feat = 1:size(FEAT,2)
    ...
    for fold = 1:n
        ...
        testLabel = disorderTruth((fold-1)*s+1:fold*s); % returns ground truth matrix [1x50] of test data (for confusion matrix)
        trainData = [FEAT(1:((fold-1)*s),feat); FEAT(fold*s+1:end,feat)]; % returns normalized feature matrix [1x450] of training data
        trainLabel = [disorderTruth(1:((fold-1)*s)); disorderTruth(fold*s+1:end)]; % returns ground truth matrix [1x450] of training data
    ...
    performance = estMat - disorderTruth;
...
confusionMatrix = confusionmat(disorderTruth, estMat)
...
%combined = [disorderTruth, FEAT];
%randomMatrix = floor(1000*rand(length(disorderTruth),1 ));
